...
describe('ReviewInput Component', () => {
  ...
  it('adds a review to the store when the form is submitted', () => {
    ...
    let input = wrapper.find(Restaurants).find('input').first()
    let form = wrapper.find(Restaurants).find('form');
    ...
    expect(wrapper.find(Reviews).html()).to.include('Better Ingredients')
  });
  ...
  it('associates the review with the restaurant with a foreign key on the review', () => {
    ...
    let reviewForm = wrapper.find(Restaurants).find('form');
    let textField =  wrapper.find(Restaurants).find('input').first();
...
describe('Reviews Component', () => {
  ...
  it('displays a review for when it is associated with the restaurant', () => {
    ...
    expect(wrapper.find(Reviews)).to.have.length(2);
  });
  ...
  it('has a button that dispatches a DELETE_REVIEW action when clicked', ()=> {
    ...
    expect(wrapper.find(Reviews).length).to.equal(0)
  });
